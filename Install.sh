...
# 安装Node.js和npm（在Termux中npm包含在nodejs中）
if ! command -v node >/dev/null 2>&1; then
    echo -e "${YELLOW}${BOLD}>> 📦 检测到未安装：Node.js，正在安装...${NC}"
    echo -e "${CYAN}${BOLD}>> 💡 提示：在Termux中，npm包含在nodejs包中，无需单独安装npm${NC}"

    # 尝试安装LTS版本，如果失败则安装普通版本
    if pkg list-all | grep -q '^nodejs-lts/'; then
        echo -e "${YELLOW}${BOLD}>> 🔄 尝试安装 nodejs-lts...${NC}"
        if ! pkg install -y nodejs-lts; then
            echo -e "${YELLOW}${BOLD}>> ⚠️ nodejs-lts 安装失败，尝试安装 nodejs...${NC}"
    ...
    if command -v node >/dev/null 2>&1 && command -v npm >/dev/null 2>&1; then
        echo -e "${GREEN}${BOLD}>> ✅ Node.js 和 npm 安装成功！${NC}"
        echo -e "${CYAN}${BOLD}>> 📋 Node.js 版本: $(node --version)${NC}"
        echo -e "${CYAN}${BOLD}>> 📋 npm 版本: $(npm --version)${NC}"
    else
        echo -e "${RED}${BOLD}>> ❌ Node.js 或 npm 安装失败！${NC}"
        echo -e "${YELLOW}${BOLD}>> 💡 常见解决方案：${NC}"
        echo -e "${YELLOW}${BOLD}   1. 运行: pkg update && pkg upgrade${NC}"
        echo -e "${YELLOW}${BOLD}   2. 重新运行: pkg install nodejs${NC}"
        echo -e "${YELLOW}${BOLD}   3. 检查存储空间是否充足${NC}"
        exit 1
    fi
else
    echo -e "${CYAN}${BOLD}>> ✅ Node.js 已安装，版本: $(node --version)${NC}"
    if command -v npm >/dev/null 2>&1; then
        echo -e "${CYAN}${BOLD}>> ✅ npm 已安装，版本: $(npm --version)${NC}"
    else
        echo -e "${RED}${BOLD}>> ❌ Node.js已安装但npm不可用，这很异常！${NC}"
        echo -e "${YELLOW}${BOLD}>> 🔧 尝试重新安装nodejs...${NC}"
        pkg install -y nodejs --reinstall
    fi
fi

# 配置npm前缀
echo -e "${CYAN}${BOLD}>> 🔧 配置npm环境...${NC}"
